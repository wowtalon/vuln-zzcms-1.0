<?PHP
	...
	*/
	class upfile{
		...
		private $file_max_size=10000000;//允许文件上传最大
		private $file_folder="uploadFiles";//文件上传路径
		private $over_write=false;//是否覆盖同名文件
		//允许上传图片的类型
		//protected $allow_type=array('image/jpg','image/jpeg','image/png','image/pjpeg','image/gif','image/bmp','image/x-png','application/octet-stream','application/zip');
			 ...
			 switch ($imgsize[2]){
					case 1:
							if(!function_exists("imagecreatefromgif")){
									showmsg("你的GD库不能使用GIF格式的图片，请使用Jpeg或PNG格式！返回");
							}
							$im = imagecreatefromgif($this->files_name());
							break;
					case 2:
							if(!function_exists("imagecreatefromjpeg")){
									showmsg("你的GD库不能使用jpeg格式的图片，请使用其它格式的图片！返回");
							}
